...
2. Provide the corrected code for that line or block."""
        ...
        self.chat_worker.chat_update.connect(self.append_chat_stream, Qt.ConnectionType.QueuedConnection)
        self.chat_worker.finished.connect(self.chat_stream_finished, Qt.ConnectionType.QueuedConnection)
        self.chat_worker.finished.connect(thread.quit)
        self.chat_worker.finished.connect(self.chat_worker.deleteLater)
        thread.finished.connect(thread.deleteLater)
        self.show_loading_indicator()
        self.chat_worker.finished.connect(self.hide_loading_indicator, Qt.ConnectionType.QueuedConnection)
        self.active_threads.append(thread)
        thread.finished.connect(
            lambda: self.active_threads.remove(thread)
            if thread in self.active_threads else None
    ...
    def _on_tree_context_menu(self, pos):
        ...
        # If nothing selected, operate on the project root
        root_path = Path(
            self.file_model.filePath(self.tree_view.rootIndex())
        )
        parent_path = (
        ...
        # Terminal — open in current dir if plugin is active
        ...
        # ── New Folder ────────────────────────────────────────────────
            ...
            if ok and name.strip():
                ...
                try:
                    ...
                    self.statusBar().showMessage(
                        f'Created folder: {new_path.name}', 3000
                    )
                except Exception as e:
                    QMessageBox.warning(
                        self, 'Error', f'Could not create folder:\n{e}'
                    )
        ...
        # ── Rename ────────────────────────────────────────────────────
            ...
            if ok and name.strip() and name.strip() != sel_path.name:
                ...
                try:
                    ...
                    self.statusBar().showMessage(
                        f'Renamed to: {new_path.name}', 3000
                    )
                    # If the file was open, reload it
                    if sel_path.is_file() or new_path.is_file():
                        self._reload_file_in_editors(
                            str(new_path)
                        )
                except Exception as e:
                    QMessageBox.warning(
                        self, 'Error', f'Could not rename:\n{e}'
                    )
        ...
        # ── Open in Terminal ──────────────────────────────────────────
            ...
            target = sel_path if sel_path and sel_path.is_dir() \
                     else parent_path
            if hasattr(self.terminal_dock, 'set_working_dir'):
                ...
                self.terminal_dock.run_command(
                    f'cd {str(target)!r}'
                )
    ...
    def _relaunch_as_agent(self, user_text: str):
        ...
        worker.finished.connect(self.chat_stream_finished, Qt.ConnectionType.QueuedConnection)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        thread.finished.connect(thread.deleteLater)
        self.show_loading_indicator()
        worker.finished.connect(self.hide_loading_indicator, Qt.ConnectionType.QueuedConnection)
        self.active_threads.append(thread)
        thread.finished.connect(
            lambda: self.active_threads.remove(thread)
            if thread in self.active_threads else None
